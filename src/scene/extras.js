...
import { loadBrandGlyph, wordmarkCanvas } from './brand.js';
...
/** Textura del wordmark calcado (dibujo compartido en brand.js). */
function neonTexture() {
  const tex = new THREE.CanvasTexture(wordmarkCanvas());
  tex.colorSpace = THREE.SRGBColorSpace;
  return tex;
}

/** El isotipo |o| calcado: barras 0.24H, aro de trazo 0.26H, gaps 0.18H. */
function ioTexture() {
  const cnv = document.createElement('canvas');
  ...
  const H = 130;                       // altura del isotipo
  const bar = Math.round(H * 0.24);    // 31
  const lw = Math.round(H * 0.26);     // 34
  const gapIo = Math.round(H * 0.18);  // 23
  const y = 128 - H / 2;
  const r = (H - lw) / 2;
  c.lineWidth = lw;
  c.beginPath(); c.arc(128, 128, r, 0, Math.PI * 2); c.stroke();
  c.fillRect(128 - H / 2 - gapIo - bar, y, bar, H);
  c.fillRect(128 + H / 2 + gapIo, y, bar, H);
  const tex = new THREE.CanvasTexture(cnv);
  tex.colorSpace = THREE.SRGBColorSpace;
  return tex;
}
...
export class Extras {
  constructor(scene) {
    ...
    this.neonMat = new THREE.MeshBasicMaterial({
      map: neonTexture(), transparent: true, depthWrite: false,
    });
